...
from loss import HairMatSoftmaxLoss, dice_loss, CrossEntropyLoss2d, iou_softmax_loss
...
class Trainer:
    ...
    def load_model(self):
        ...
        model = glob(os.path.join(self.model_path, "PrismaNet_portrait*.pth"))
        model.sort()
    ...
    def train(self):
        MobileHairNetSoftmaxLoss = HairMatSoftmaxLoss().to(self.device)
        ...
        for epoch in range(self.epoch):
            for step, (image, mask) in enumerate(self.data_loader):
                ...
                pred = self.net(image)
                self.net.zero_grad()
                loss = MobileHairNetSoftmaxLoss(pred, image, mask, gradient_loss_ratio=0.0)
                loss.backward()
                optimizer.step()
                ...
                if step % self.sample_step == 0:
                    # we do not use argmax here for it introduce hard mask edge
                    # self.save_sample_imgs(image[0], mask[0], torch.argmax(pred[0], 0, keepdim=True), self.sample_dir, epoch, step)
                    self.save_sample_imgs(image[0], mask[0], pred[0], self.sample_dir, epoch, step)
                    
                    print('[*] Saved sample images')

            torch.save(self.net.state_dict(), '%s/PrismaNet_portrait_epoch-%04d.pth' % (self.checkpoint_dir, epoch))

    def save_sample_imgs(self, real_img, real_mask, prediction, save_dir, epoch, step):
        data = [real_img, real_mask, prediction[:,:,1]] # took out 1 channel of softmax result
        ...
        for i, d in enumerate(data):
            ...
            if i == 1 or i == 2: # real_mask and pred_mask only has one channel, stack them for img
