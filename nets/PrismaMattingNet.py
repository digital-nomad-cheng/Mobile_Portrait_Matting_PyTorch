"""
Implementation of PrismaNet which I found on the blog of Prisma + mobile matting block which use sigmoid as alpha weight
"""
import re
import math
import argparse
...
class PrismaNet(nn.Module):
    ...
    def __init__(self):
        ...
        # feathering block
        self.conv_f1 = conv_bn(11, 32, 1, True)
        self.conv_f2 = conv_bn(32, 3, 1, True)
    ...
    def forward(self, x):
        ...
        # ------------------ feathering block --------------- # 
        background, foreground = torch.split(up4, 1, dim=1)
        x_square = x0*x0
        foreground3 = torch.cat((foreground, foreground, foreground), 1)
        x_masked = x0*foreground3

        feather_input = torch.cat((x0, up4, x_square, x_masked), 1)
        d7 = self.conv_f1(feather_input)
        d8 = self.conv_f2(d7)
        a, b, c = torch.split(d8, 1, dim=1)
        a = a*background
        b = b*foreground
        c = a+b+c
        
        output = self.sigmoid(c)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_model_path', type=str, default='models/PrismaNe2.pth', help='input model path')
